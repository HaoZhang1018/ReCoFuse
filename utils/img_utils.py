...
def crop_to_original_size(img_tensor, original_height, original_width):
    """
    Crop (B,C,H,W) or (C,H,W) tensor to (C,H,W) with desired size.
    """
    if img_tensor.dim() == 4:
        ...
        raise ValueError("Unsupported tensor shape: expected 3D or 4D tensor.")
...
def calculate_entropy_and_std(tensor_img):
    """
    输入：PyTorch Tensor，shape 为 (C,H,W) 或 (B,C,H,W)
    功能：
        - 自动转为 numpy 格式
        - 自动 squeeze
        - 自动判断 RGB or 灰度并转换为 img_gray
        - 计算 entropy 和标准差
    返回：
        entropy_val: float
        std_val: float
    ...
    # Step 4: histogram and entropy
    hist = cv2.calcHist([img_gray], [0], None, [256], [0, 256]).flatten()
    hist_norm = hist / np.sum(hist)
    entropy_val = entropy(hist_norm + 1e-12, base=2)  # 避免 log(0)
    std_val = np.std(img_gray)

    return entropy_val, std_val