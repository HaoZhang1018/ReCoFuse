...
def check_resume(opt, resume_iter):
    ...
    if opt["path"]["resume_state"]:
        if (
            ...
            or opt["path"].get("pretrain_model_F", None) is not None
        ):
            logger.warning(
        ...
        opt["path"]["pretrain_model_F"] = osp.join(
            opt["path"]["models"], "{}_F.pth".format(resume_iter)
        )        
        logger.info("Set [pretrain_model_G] to " + opt["path"]["pretrain_model_G"])
        logger.info("Set [pretrain_model_F] to " + opt["path"]["pretrain_model_F"])
        if "gan" in opt["model"]:
